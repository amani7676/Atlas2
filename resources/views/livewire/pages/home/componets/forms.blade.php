<div class="card {{ $isExpanded ? 'h-100' : '' }}">
    <div class="card-header card-header-forms d-flex justify-content-between align-items-center" style="cursor: pointer;" wire:click="toggle">
            ...
            <table class="table table-hover table-sm mb-0 conditional-scroll-table">
                ...
                <tbody>
                    ...
                    @foreach ($this->allReportService->getAllResidentsWithDetails() as $data)
                       @if (isset($data['resident']) && $data['resident'] !== null && !($data['resident']['form'] ?? false))
                            @php
                                $counter++;
                            @endphp
                            <tr>
